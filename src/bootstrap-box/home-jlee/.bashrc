...
#   export PS1='\[\033[01;32m\]\u@\h\[\033[01;34m\]\w \$ \[\033[00m\]'
    ...
    alias tacc='tail -f /var/log/apache2/access_log'
    alias terr='tail -f /var/log/apache2/error_log'
    ...
    function mgrep() { grep -r $* . | egrep -v '.svn|.pyc|Binary' | grep -vG '^.*\.svn.*:.*$' | grep -vG '^.*.pyc.*:.*$' ; }
    ...
    function cvd() {
     v=`cvs log $1 |grep rev|head -3|tail -1|cut -f2 -d' '`
     /usr/bin/cvs diff -r $v $1
    }
...
if [ "$1" == "up" ];then
/usr/bin/cvs $* 2>&1| /bin/egrep -iv "^cvs server|? batch|? util|^cvs update: Updating|cvs update: cannot open directory|directory CVS|CVS directory|\.Thumb|\.swp|? dump|? cmd/|\.project|\.cache|\.settings|\.buildpath"
return;
fi
if [ "$1" != "ci" ] && [ "$1" != "commit" ]
then
/usr/bin/cvs $* 2>&1|egrep -iv "^cvs server|? batch|? util|^cvs update: Updating|cvs update: cannot open directory|directory CVS|CVS directory|.Thumb|\.swp|? dump|? cmd/|\.project|\.cache|\.settings|\.buildpath"
else
/usr/bin/cvs $* #2>&0 |egrep -iv "cvs commit: Examining|cvs commit: but CVS|cvs commit: warning: directory CVS"
fi
}
