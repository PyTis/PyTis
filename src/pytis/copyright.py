...
def javascript(fi,cp):
	...
	try:
		...
		lines = handle.readlines(-1)
		handle.close()
		nhandle = open(fi,'wb', encoding='utf-8')
...
def fileType(fi):
	...
	def tryShebang(fi):
		...
		handle = open(os.path.abspath(fi),'rb')
		lines = handle.readlines(-1)
		if lines:
			line = lines[0]
...
License."""
	...
	handle = open(fi, 'wb', encoding='utf-8')
...
def setConfig(d):
	...
	handle = open(mpath,'wb', encoding='utf-8')
